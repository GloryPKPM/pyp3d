...
class GeTransform(BufferStackBase):
    ...
    def __mul__(self, b): 
        if isinstance(b, (list, tuple)): return [self * i for i in b]
    ...
    def __add__(self, b):
        if isinstance(b, GeTransform): return GeTransform([[self._mat[j][i]+b._mat[j][i] for i in range(4)] for j in range(3)])
...
class GeVec3d(BufferStackBase):
    ...
    def __rmul__(self, a):
        ...
        elif isinstance(a, (float, int)): return GeVec3d(a*self.x, a*self.y, a*self.z)
    ...
    def __neg__(self):
        ...
        return c
    def __mul__(self, b):
        if isinstance(b, (float, int)):return GeVec3d(self.x * b,self.y * b,self.z * b) 
        else: raise TypeError('improper parameter!')
    def __add__(self, b):
...
class GeVec2d(BufferStackBase):
    ...
    def __rmul__(self, a):
        ...
        elif isinstance(a, (float, int)): return GeVec2d(a*self.x, a*self.y)
        else: raise TypeError('input error type!')
    def __neg__(self):
        c = copy.deepcopy(self)
        ...
        return c
    def __mul__(self, b):
        if isinstance(b, (float, int)):return GeVec2d(self.x * b,self.y * b) 
        else: raise TypeError('improper parameter!')
    def __add__(self, b):
